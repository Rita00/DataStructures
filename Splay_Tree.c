...
palavra splay_tree(palavra new_node);
...
palavra splay_tree(palavra new_node) {
    while (new_node->pai != NULL) {
        if (new_node->pai->pai == NULL) { /*operacoes de ZIG*/
            if (new_node == new_node->pai->left) {
                /*ZIG esquerda (rotacao a direita)*/
                roda_direita(new_node->pai);
            } else if (new_node == new_node->pai->right) {
                /*ZIG a direita (rotacao a esquerda)*/
                roda_esq(new_node->pai);
            }
        } else if (new_node == new_node->pai->right && new_node->pai == new_node->pai->pai->right) {
            /*ZIG ZIG a direita (rotacao a esquerda)*/
            roda_esq(new_node->pai->pai);
            roda_esq(new_node->pai);
        } else if (new_node == new_node->pai->left && new_node->pai == new_node->pai->pai->left) {
            /*ZIG ZIG a esquerda (rotacao a direita)*/
            roda_direita(new_node->pai->pai);
            roda_direita(new_node->pai);
        } else if (new_node == new_node->pai->left && new_node->pai == new_node->pai->pai->right) {
            /*ZIG ZAG (rotacao a direita seguida de rotacao a esquerda)*/
            roda_direita(new_node->pai);
            roda_esq(new_node->pai);
        } else if (new_node == new_node->pai->right && new_node->pai == new_node->pai->pai->left) {
            /*ZIG ZAG (rotacao a esquerda seguida de rotacao a direita)*/
            roda_esq(new_node->pai);
            roda_direita(new_node->pai);
        }
    }
    ...
    while (strcasecmp(buffer, "TCHAU") != 0) {
        if (strcasecmp(buffer, "TEXTO") == 0) {
            ...
            if ((res_pesq = pesquisa_palavra(word, BDpalavras)) != NULL) {
                BDpalavras = splay_tree(res_pesq);
